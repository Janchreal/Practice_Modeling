// 本文件：构造/析构、窗口与鼠标事件、工作轴/基准平面、点选原点、历史形状访问等（VTK 左键路由见 widget_vtk_click.cpp）。
// 头文件仅保留本翻译单元实际用到的类型（widget.h 已含大量 OCC/VTK/Qt 公共依赖）。
#include "widget.h"
#include "widget_mirror_types.h"
#include "widget_mirror_globals.h"
#include "ui_widget.h"

#include "nx_main_menu_builder.h"
        ...
        for (IVtk_ShapeIdList::Iterator sIt(subShapeIds); sIt.More(); sIt.Next()) {
            ...
            if (subShape.ShapeType() == TopAbs_VERTEX) {
                ...
                if (!curve.IsNull()) {
                    ...
                    selectedPoint = curve->Value(midParam);
                    pointType = "中点";
                    pointFound = true;
                    break;
                }
...
TopoDS_Shape Widget::getShapeFromHistory(int index)
{
    ...
    switch (record.type) {
    case CUBOID:
        return createCuboidShape(record.param1, record.param2, record.param3,
    ...
    case CYLINDER:
        return createCylinderShape(record.param1, record.param2,
                                   record.hasOrigin, record.originX, record.originY, record.originZ,
                                   record.axisDirection,
                                   record.axisReversed,
                                   record.hasCustomVectorDir,
                                   record.customVectorDir);
    case CONE:
        return createConeShape(record.param1, record.param2, record.param3,
                               record.hasOrigin, record.originX, record.originY, record.originZ,
                               record.axisDirection,
                               record.axisReversed,
                               record.hasCustomVectorDir,
                               record.customVectorDir);
    case SPHERE:
        return createSphereShape(record.param1, record.hasOrigin, record.originX, record.originY, record.originZ);
    case BOOLEAN_RESULT:
