...
#include "presentation/dialogs/modification/fillet_dialog.h"
#include "presentation/dialogs/modification/chamfer_dialog.h"
...
void Widget::on_fillet_clicked()
{
    ...
    connect(dialog, &QDialog::accepted, this, [this, dialog]() {
        ...
        try {
            TopoDS_Shape originalShape = geometryStateFor(historyList[filletTargetModelIndex]).occShape;
            ...
            const TopoDS_Shape& parentShape = geometryStateFor(historyList[filletTargetModelIndex]).occShape;
...
bool Widget::buildFilletPreviewShape(TopoDS_Shape& outShape) const
{
    ...
    const TopoDS_Shape originalShape = geometryStateFor(historyList[filletTargetModelIndex]).occShape;
...
bool Widget::buildChamferPreviewShape(TopoDS_Shape& outShape) const
{
    ...
    const TopoDS_Shape originalShape = geometryStateFor(historyList[chamferTargetModelIndex_]).occShape;
    if (originalShape.IsNull()) return false;
...
void Widget::on_chamfer_clicked()
{
    ...
    connect(dialog, &QDialog::accepted, this, [this, dialog]() {
        ...
        try {
            ...
            const TopoDS_Shape& parentShape = geometryStateFor(historyList[chamferTargetModelIndex_]).occShape;
...
// ─── 倒角/圆角拖拽手柄（中心球 + 圆柱导向杆 + 贴面锥体） ────────────────

namespace {

constexpr double kChamferArrowLen = 0.55; // 与拉伸手柄同量级（屏幕恒定尺寸）
constexpr double kChamferConeRadiusFactor = 0.155;
constexpr double kChamferGuideCylinderR = 0.016;
constexpr double kChamferSphereR = 0.055;
...
void setChamferArrowGeometry(vtkActor* actor, const gp_Pnt& baseCenter, const gp_Dir& dir, double coneLen)
{
    if (!actor) return;
    HandleGeom::ConeParams params;
    params.height = std::max(1e-6, coneLen);
    params.radius = std::max(1e-6, coneLen * kChamferConeRadiusFactor);
    params.resolution = 28;
    params.capping = true;
    auto cone = HandleGeom::makeConeWithBaseCenter(baseCenter, dir, params);
...
void setChamferLineGeometry(vtkActor* actor, const gp_Pnt& a, const gp_Pnt& b, double radius)
{
    if (!actor) return;
    HandleGeom::CylinderParams params;
    params.radius = std::max(1e-6, radius);
    params.resolution = 18;
    params.capping = true;
    auto tube = HandleGeom::makeCylinderBetween(a, b, params);
    if (auto* mapper = vtkPolyDataMapper::SafeDownCast(actor->GetMapper())) {
...
{
    HandleGeom::ConeParams params;
    params.height = kChamferArrowLen;
    params.radius = kChamferArrowLen * kChamferConeRadiusFactor;
    params.resolution = 28;
    auto cone = HandleGeom::makeConeWithBaseCenter(gp_Pnt(0, 0, 0), gp_Dir(1, 0, 0), params);
...
{
    HandleGeom::CylinderParams params;
    params.radius = kChamferGuideCylinderR;
    params.resolution = 18;
    auto tube = HandleGeom::makeCylinderBetween(gp_Pnt(0, 0, 0), gp_Pnt(1e-6, 0, 0), params);
    auto mapper = vtkSmartPointer<vtkPolyDataMapper>::New();
    mapper->SetInputConnection(tube->GetOutputPort());
    auto actor = vtkSmartPointer<vtkActor>::New();
    actor->SetMapper(mapper);
...
vtkSmartPointer<vtkActor> makeChamferSphereActor()
{
    auto sphere = HandleGeom::makeSphereSource(
        HandleGeom::defaultHandleSphereParams(kChamferSphereR));
    auto mapper = vtkSmartPointer<vtkPolyDataMapper>::New();
...
void setChamferSphereGeometry(vtkActor* actor, const gp_Pnt& center, double worldScale)
{
    if (!actor) return;
    actor->SetPosition(center.X(), center.Y(), center.Z());
    actor->SetScale(worldScale, worldScale, worldScale);
}

void styleChamferAnchorSphere(vtkActor* sphere, bool active, bool hover)
{
    if (!sphere) return;
    const double r = active ? kChamferDragR : hover ? kChamferHoverR : kChamferBlueR;
    const double g = active ? kChamferDragG : hover ? kChamferHoverG : kChamferBlueG;
    const double b = active ? kChamferDragB : hover ? kChamferHoverB : kChamferBlueB;
    vtkProperty* p = sphere->GetProperty();
    p->SetColor(r, g, b);
    p->SetAmbient(0.9);
    p->SetDiffuse(0.3);
    p->SetSpecular(0.08);
...
void Widget::updateChamferAsymHandles()
{
    ...
    const bool side2Active = isAsym && chamferAsymHandleDragging_ && chamferAsymActiveSide_ == 1;
    const bool side1Hover = !chamferAsymHandleDragging_ && chamferAsymHoverSide_ == 0;
    const bool side2Hover = isAsym && !chamferAsymHandleDragging_ && chamferAsymHoverSide_ == 1;
    const bool anchorActive = side1Active || side2Active;
    const bool anchorHover = side1Hover || side2Hover;
    ...
    auto lineRadiusAt = [&](const gp_Pnt& tip, bool active, bool hover) {
        const double factor = active ? kChamferLineDragWidth / kChamferLineDefaultWidth
                                     : hover ? kChamferLineHoverWidth / kChamferLineDefaultWidth
                                             : 1.0;
        const gp_Pnt mid((midPt.X() + tip.X()) * 0.5,
                         (midPt.Y() + tip.Y()) * 0.5,
                         (midPt.Z() + tip.Z()) * 0.5);
        return kChamferGuideCylinderR * factor * overlayWorldScaleAt(mid.X(), mid.Y(), mid.Z());
    };

    auto sphereScaleAt = [&](bool active, bool hover) {
        const double factor = active ? kChamferArrowDragLenFactor
                                     : hover ? kChamferArrowHoverLenFactor
                                             : 1.0;
        return factor * overlayWorldScaleAt(midPt.X(), midPt.Y(), midPt.Z());
    };

    // 拖拽中：只更新几何，不销毁 Actor
    const bool canUpdateInPlace = chamferAsymHandleDragging_
        && chamferAsymHandleSphereActor_
    ...
    if (canUpdateInPlace) {
        setChamferSphereGeometry(chamferAsymHandleSphereActor_, midPt, sphereScaleAt(anchorActive, false));
        styleChamferAnchorSphere(chamferAsymHandleSphereActor_, anchorActive, false);
        setChamferLineGeometry(chamferAsymHandleLine1Actor_, midPt, tip1,
                               lineRadiusAt(tip1, side1Active, false));
        ...
        if (isAsym) {
            setChamferLineGeometry(chamferAsymHandleLine2Actor_, midPt, tip2,
                                   lineRadiusAt(tip2, side2Active, false));
    ...
    chamferAsymHandleSphereActor_ = makeChamferSphereActor();
    setChamferSphereGeometry(chamferAsymHandleSphereActor_, midPt, sphereScaleAt(anchorActive, anchorHover));
    styleChamferAnchorSphere(chamferAsymHandleSphereActor_, anchorActive, anchorHover);
    addReferenceActor(chamferAsymHandleSphereActor_);

    chamferAsymHandleLine1Actor_ = makeChamferLineActor();
    chamferAsymHandleSide1Actor_ = makeChamferArrowActor();
    setChamferLineGeometry(chamferAsymHandleLine1Actor_, midPt, tip1,
                           lineRadiusAt(tip1, side1Active, side1Hover));
    setChamferArrowGeometry(chamferAsymHandleSide1Actor_, tip1, chamferAsymDir1_,
    ...
    if (isAsym) {
        ...
        setChamferLineGeometry(chamferAsymHandleLine2Actor_, midPt, tip2,
                               lineRadiusAt(tip2, side2Active, side2Hover));
        setChamferArrowGeometry(chamferAsymHandleSide2Actor_, tip2, chamferAsymDir2_,
...
void Widget::clearChamferAsymHandles()
{
    ...
    remove(chamferAsymHandleSphereActor_);
...
void Widget::updateFilletRadiusHandles()
{
    ...
    const bool canUpdateInPlace = filletRadiusHandleDragging_
        && filletRadiusHandleSphereActor_
        && filletRadiusHandleSide1Actor_ && filletRadiusHandleLine1Actor_
        && filletRadiusHandleSide2Actor_ && filletRadiusHandleLine2Actor_;

    if (canUpdateInPlace) {
        setChamferSphereGeometry(filletRadiusHandleSphereActor_, midPt, sphereScaleAt(anchorActive, false));
        styleChamferAnchorSphere(filletRadiusHandleSphereActor_, anchorActive, false);
    ...
    filletRadiusHandleSphereActor_ = makeChamferSphereActor();
    setChamferSphereGeometry(filletRadiusHandleSphereActor_, midPt, sphereScaleAt(anchorActive, anchorHover));
    styleChamferAnchorSphere(filletRadiusHandleSphereActor_, anchorActive, anchorHover);
    addReferenceActor(filletRadiusHandleSphereActor_);
...
void Widget::clearFilletRadiusHandles()
{
    ...
    remove(filletRadiusHandleSphereActor_);
