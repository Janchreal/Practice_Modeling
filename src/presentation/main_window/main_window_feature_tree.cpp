// 特征树、可见性、右键/上下文菜单（从 main_window.cpp 拆出）
...
#include "application/commands/regeneratemodelcommand.h"
...
void Widget::updateFeatureTree()
{
    ...
    for (int i = 0; i < historyList.size(); ++i) {
        ...
        if (renderStateFor(historyList[i]).actor) {
            isVisible = (renderStateFor(historyList[i]).actor->GetVisibility() != 0);
        }
...
void Widget::onFeatureTreeContextMenuRequested(const QPoint& pos)
{
    ...
    } else if (renderStateFor(record).actor) {
        isVisible = (renderStateFor(record).actor->GetVisibility() != 0);
    }
...
void Widget::showContextMenu(int x, int y, int modelIndex, const QPoint& globalPos)
{
    ...
    if (modelIndex >= 0 && modelIndex < historyList.size()) {
        ...
        // 根据当前可见性显示"隐藏"或"显示"
        bool isVisible = true;
        if (renderStateFor(historyList[modelIndex]).actor) {
            isVisible = (renderStateFor(historyList[modelIndex]).actor->GetVisibility() != 0);
        }
