...
/** vtkConeSource 几何参数（height 为底面到尖端的长度） */
struct ConeParams {
    double height = 0.40;
    double radius = 0.09;
    int resolution = 24;
...
/** 圆柱导向杆参数（用 vtkTubeFilter 沿两点连线生成） */
...
/** 生成锥体；baseCenter 为底面中心，dir 为底面指向尖端的方向 */
vtkSmartPointer<vtkConeSource> makeConeWithBaseCenter(
    const gp_Pnt& baseCenter,
    const gp_Dir& dir,
    const ConeParams& params = ConeParams());

/** 生成两点之间的圆柱导向杆；短线段会自动补一个极小长度以保证 VTK 管线稳定 */
vtkSmartPointer<vtkTubeFilter> makeCylinderBetween(
    const gp_Pnt& a,
    const gp_Pnt& b,
    const CylinderParams& params = CylinderParams());
