...
void Widget::handleVtkMouseClick(int x, int y)
{
    ...
    if (currentSelectionMode == None) {
        ...
        if (strictIdx >= 0 && strictIdx < historyList.size() && renderStateFor(historyList[strictIdx]).actor) {
    ...
    if (!shapePicker) {
        if (earlyModelIdx >= 0 && earlyModelIdx < historyList.size() && renderStateFor(historyList[earlyModelIdx]).actor) {
    ...
    // 拾取前绑定当前上下文并更新 ShapeDataSource，确保 shapePicker 能识别所有模型。
    ...
    if (selectedIndex < 0
        && !(currentSelectionMode == PointSelection && revolveDialog)
        ...
        if (fb >= 0 && fb < historyList.size() && renderStateFor(historyList[fb]).actor) {
            selectedIndex = fb;
            selectedActor = renderStateFor(historyList[fb]).actor;
        }
    }
    ...
    if (currentSelectionMode == PointSelection && revolveDialog) {
        if (selectedActor) {
        ...
        gp_Pnt picked;
        const bool useSketchPlane = inSketchEnvironment_ && hasActiveSketch_;
        const gp_Pln pickPlane = useSketchPlane
            ? activeSketchPlane_
            : gp_Pln(gp_Pnt(0, 0, 0), gp_Dir(0, 0, 1));
        if (!tryPickPointOnPlane(pickPlane, x, y, picked)) {
            if (statusBar()) {
                statusBar()->showMessage(tr("旋转中心：无法在当前工作平面取点，请调整视角后重试。"), 3000);
            }
            return;
        }
        ...
        const QString pointType = useSketchPlane ? tr("草图平面点") : tr("基准平面点");
