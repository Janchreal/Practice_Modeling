...
static HandleSpec buildExtrusionDistanceSpec()
{
    ...
    // 控制柄 2：终止锥体（底面贴合终止面，可拖拽）
    {
        ControlSpec c;
        c.id = QStringLiteral("end_arrow");
        c.shape = ControlShape::Cone;
        c.draggable = true;
        applyStandardFourStates(c, 2.0f);
...
static HandleSpec buildRevolveAngleSpec()
{
    ...
    // 导引角色 4：旋转中心球（不可交互，标识用户指定的旋转轴基点）
    {
        GuideSpec g = makeGuideSpec(
            QStringLiteral("revolve_center"),
            QStringLiteral("旋转中心（用户指定的旋转轴基点）"));
...
/**
 * R-Handle-VectorTwoPoint
 * 两点定矢量：起/终点球形控制柄 + 方向箭头（双击反转）
 */
static HandleSpec buildVectorTwoPointSpec()
{
    HandleSpec h;
    h.id = QStringLiteral("vector_two_point");
    h.type = HandleType::Vector;
    h.featureType = QStringLiteral("Generic");
    h.cond = []{ return true; };

    auto addSphere = [&](const QString& id) {
        ControlSpec c;
        c.id = id;
        c.shape = ControlShape::Sphere;
        c.draggable = true;
        applyStandardFourStates(c);
        h.controls.append(c);
    };

    addSphere(QStringLiteral("start_sphere"));
    addSphere(QStringLiteral("end_sphere"));
...
void registerAllHandleSpecs()
{
    ...
    reg.registerSpec(buildVectorTwoPointSpec());
}
