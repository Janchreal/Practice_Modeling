...
double computeDatumHalfSize(const QList<ModelingHistory>& historyList,
                            ...
                            const ModelRenderStore& renderStore)
{
    ...
    if (currentSelectedIndex >= 0 && currentSelectedIndex < historyList.size()) {
        const ModelRenderState* state = renderStore.find(historyList[currentSelectedIndex].id);
        tryActorBounds(state ? state->actor : nullptr);
    }
    if (!hasBounds) {
        for (const auto& h : historyList) {
            const ModelRenderState* state = renderStore.find(h.id);
            tryActorBounds(state ? state->actor : nullptr);
        }
    }
                               ...
                               QString* errorMessage)
{
    ...
    const double size = computeDatumHalfSize(historyList, currentSelectedIndex, snapPoints, renderStore);
