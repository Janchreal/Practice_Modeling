...
gp_Pnt Widget::revolveVectorArrowOrigin() const
{
    ...
    return getRevolutionAxis(revolveDialog).Location();
}
...
void Widget::clearVectorDialogHoverShape()
{
    ...
    if (vectorDialogHoverModelIndex_ >= 0 && vectorDialogHoverModelIndex_ < historyList.size()) {
        ...
        if (renderStateFor(rec).highlightActor) {
            renderStateFor(rec).highlightActor->SetVisibility(false);
        }
    }
    ...
    if (vectorDialogHoverDimModelIndex_ >= 0 && vectorDialogHoverDimModelIndex_ < historyList.size()) {
        ...
        if (renderStateFor(dimRec).actor && renderStateFor(dimRec).actor->GetVisibility() != 0) {
            ...
            renderStateFor(dimRec).actor->GetProperty()->SetOpacity(vectorDialogHoverDimOriginalOpacity_);
        }
...
void Widget::applyVectorDialogHoverSubShape(int modelIndex, IVtk_IdType subShapeId, bool isFace)
{
    ...
    if (renderStateFor(rec).shapeWrapper.IsNull()) return;
    ...
    TopoDS_Shape sh;
    try {
        sh = renderStateFor(rec).shapeWrapper->GetSubShape(subShapeId);
    ...
    if (isFace) {
        ...
        if (renderStateFor(rec).actor && vectorDialogHoverDimModelIndex_ != modelIndex) {
            ...
            vectorDialogHoverDimOriginalOpacity_ = renderStateFor(rec).actor->GetProperty()->GetOpacity();
...
void Widget::applyOriginFromSnap(const gp_Pnt& p, const QString& chosenLabel)
{
    ...
    const bool refreshRevolveAfterOriginPick =
        pendingOriginDialogKind_ == OriginDialogKind::Revolve && revolveDialog;
    ...
    if (refreshRevolveAfterOriginPick) {
        QTimer::singleShot(0, this, [this]() {
            if (!revolveDialog) return;
            updateRevolveHandles();
            refreshRevolveLivePreview();
        });
    }
                                                    ...
                                                    QList<SketchSnapScreenCandidate>& out) const
{
    ...
    for (int hi = 0; hi < historyList.size(); ++hi) {
        ...
        appendEdgesFromShape(geometryStateFor(rec).occShape, edges);
    }
    ...
    if (snap_.endpoint) {
        ...
        for (int hi = 0; hi < historyList.size(); ++hi) {
            ...
            if (geometryStateFor(rec).occShape.IsNull()) continue;
            if (hasActiveSketch_ && hi == activeSketchHistoryIndex_) continue;
            addVertices(geometryStateFor(rec).occShape);
        }
    }
}
...
void Widget::updateSnapHover(int x, int y, SnapHoverOptions options)
{
    ...
    for (int i = 0; i < historyList.size(); ++i) {
        if (renderStateFor(historyList[i]).actor && renderStateFor(historyList[i]).shapeDataSource) {
            const bool visible = (renderStateFor(historyList[i]).actor->GetVisibility() != 0);
            renderStateFor(historyList[i]).actor->SetPickable(visible);
            if (visible) {
                rebindHistoryShapeSource(historyList[i]);
            } else {
                IVtkTools_ShapeObject::SetShapeSource(nullptr, renderStateFor(historyList[i]).actor);
    ...
    if (options.expandScreenPixelRadius > 0.0) {
        ...
        for (const auto& rec : historyList) {
            ...
            if (snap_.endpoint) {
                for (TopExp_Explorer exV(geometryStateFor(rec).occShape, TopAbs_VERTEX); exV.More(); exV.Next()) {
