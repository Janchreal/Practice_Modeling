...
void Widget::on_fillet_clicked()
{
    ...
    connect(dialog, &QDialog::accepted, this, [this, dialog]() {
        ...
        try {
            ...
            BRepFilletAPI_MakeFillet fillet(originalShape);
            const bool isG2 = dialog->continuityText().startsWith("G2");
            const double rhoRaw = dialog->rhoValue();
            const double rho = std::clamp(rhoRaw, 0.05, 1.95);
            const double rhoN = std::clamp((rho - 0.05) / 1.90, 0.0, 1.0); // 归一化到 [0,1]

            auto applyProfile = [&](int profileIndex) {
                if (!isG2) {
                ...
                // 严格 G2（不降级）：Rho 驱动截面与逼近参数，逼近 NX Conic Rho 的“偏椭圆-抛物-偏双曲”调节感受。
                switch (profileIndex) {
                case 0: // 主策略：rho<1 偏 Polynomial，rho>=1 偏 QuasiAngular
                    ...
                    break;
                case 1: // 次策略：互换形态，覆盖另一侧 conic 风格
                    ...
                    break;
                default: // 兜底：保持 G2 与 Rho，但明显放宽逼近
                    fillet.SetFilletShape(ChFi3d_Rational);
            ...
            auto tryBuild = [&](double radius, int profileIndex) -> bool {
                fillet.Reset();
            ...
            // 类 NX：在目标连续性下做多策略 + 半径退让；若 G2 失败则保持失败，不降级到 G1。
            const int maxProfiles = isG2 ? 3 : 1;
            for (int p = 0; p < maxProfiles && !ok; ++p) {
                for (int attempt = 0; attempt < 6; ++attempt) {
                    usedR = r * std::pow(0.5, attempt);
                    if (usedR <= 1e-9) break;
                    if (tryBuild(usedR, p)) {
                        ok = true;
                        break;
            ...
            TopoDS_Shape resultShape = fillet.Shape();
...
// 执行倒角操作
void Widget::performFillet(double radius)
{
    try {
        ...
        for (edgeExplorer.Init(originalShape, TopAbs_EDGE); edgeExplorer.More(); edgeExplorer.Next()) {
        ...
        if (edgeCount == 0) {
            QMessageBox::warning(this, "警告", "未找到可倒角的边！");
            return;
        }
        ...
        // 显示结果
        QString originalName = historyList[currentSelectedIndex].name;
        QString resultName = QString("倒角_%1").arg(originalName);
        executeCommand(new AddModelCommand(this, resultShape, resultName, FILLET, QColor(255, 140, 0), radius));
...
bool Widget::buildFilletPreviewShape(TopoDS_Shape& outShape) const
{
    ...
    try {
        ...
        if (!isG2) {
            ...
            const double rho = std::clamp(filletDialog->rhoValue(), 0.05, 1.95);
            const double rhoN = std::clamp((rho - 0.05) / 1.90, 0.0, 1.0);
            fillet.SetFilletShape(rho < 1.0 ? ChFi3d_Polynomial : ChFi3d_QuasiAngular);
            fillet.SetContinuity(GeomAbs_C2, 7.0e-4 + (1.0 - rhoN) * 7.0e-4);
        }
        for (const TopoDS_Edge& e : filletSelectedEdges_) {
            if (!e.IsNull()) fillet.Add(r, e);
        }
        fillet.Build();
        if (!fillet.IsDone()) {
            for (int attempt = 1; attempt <= 4; ++attempt) {
                const double rr = r * std::pow(0.5, attempt);
                if (rr <= 1e-9) break;
                fillet.Reset();
                if (!isG2) {
                    fillet.SetFilletShape(ChFi3d_Rational);
...
bool Widget::buildChamferPreviewShape(TopoDS_Shape& outShape) const
{
    ...
    try {
        BRepFilletAPI_MakeChamfer mk(originalShape);
        ...
        if (twoDistances) {
            ...
            for (const TopoDS_Edge& e : chamferSelectedEdges_) {
                ...
                const TopTools_ListOfShape& faces = edgeFaceMap.FindFromKey(e);
...
void Widget::performHollow(double thickness)
{
    try {
        ...
        hollowMaker.MakeThickSolidByJoin(originalShape, facesToRemove, thickness, 1e-3);
        ...
        TopoDS_Shape resultShape = hollowMaker.Shape();
