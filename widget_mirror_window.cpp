...
void Widget::prepareShapePickerBindingsForCurrentContext()
{
    ...
    auto unbindMainActors = [this]() {
        for (int i = 0; i < historyList.size(); ++i) {
            ...
            historyList[i].actor->SetPickable(0);
    ...
    if (g_mirrorRenderContextMap.contains(this)) {
        ...
        for (auto it = map.begin(); it != map.end(); ++it) {
            ...
            for (int i = 0; i < historyList.size(); ++i) {
                ...
                if (!mirrorActor || !historyList[i].shapeDataSource) continue;

                const bool visible = (historyList[i].actor && historyList[i].actor->GetVisibility() != 0);
                ...
                if (visible) {
                    IVtkTools_ShapeObject::SetShapeSource(historyList[i].shapeDataSource, mirrorActor);
                } else {
                    IVtkTools_ShapeObject::SetShapeSource(nullptr, mirrorActor);
                }
            }
...
int Widget::resolveHistoryIndexByActor(vtkActor* actor) const
{
    if (!actor) return -1;
    for (int i = 0; i < historyList.size(); ++i) {
        if (historyList[i].actor == actor) return i;
    }
...
void Widget::syncMirrorWindows()
{
    ...
    for (int i = windows.size() - 1; i >= 0; --i) {
        ...
        for (const ModelingHistory& record : historyList) {
            auto actor = buildMirrorActorFromHistory(record);
            if (actor) {
                const bool visible = (record.actor && record.actor->GetVisibility() != 0);
