                                               ...
                                               const QList<int>& indices)
{
    ...
    for (int index : indices) {
        ...
        const TopoDS_Shape& shape = historyList[index].occShape;
        if (!shape.IsNull()) {
            shapes.append(shape);
        }
    }
...
void Widget::updatePatternPreview()
{
    ...
    const QList<TopoDS_Shape> sourceShapes = collectPatternSourceShapes(historyList, patternSelectedIndices_);
