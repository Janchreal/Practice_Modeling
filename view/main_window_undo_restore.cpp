                                   ...
                                   const gp_Dir& customVectorDir)
{
    try {
        ...
        switch (type) {
        case CUBOID:
            shape = PrimitiveGeometry::buildCuboidShape(param1, param2, param3, hasOrigin, originX, originY, originZ,
                                                         axisDirection, axisReversed,
                                                         hasCustomVectorDir, customVectorDir);
            break;
        case CYLINDER:
            shape = PrimitiveGeometry::buildCylinderShape(param1, param2, hasOrigin, originX, originY, originZ,
                                                          axisDirection, axisReversed,
                                                          hasCustomVectorDir, customVectorDir);
            break;
        case CONE:
            shape = PrimitiveGeometry::buildConeShape(param1, param2, param3, hasOrigin, originX, originY, originZ,
                                                      axisDirection, axisReversed,
                                                      hasCustomVectorDir, customVectorDir);
            break;
        case SPHERE:
            shape = PrimitiveGeometry::buildSphereShape(param1, hasOrigin, originX, originY, originZ);
            break;
        default:
            return -1;
        }
