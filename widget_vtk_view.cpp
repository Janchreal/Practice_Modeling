// VTK setup, center triad, work CSYS, oriented views (split from widget.cpp)
#include "widget.h"
...
{
    // TabWidget 里的“定向视图”按钮（widget.ui 内的 objectName）
    ...
    // 4) 仅棱角小半径辅助（≤12px）。禁止 110px 大吸附，否则点空白也会“选中”。
    {
        ...
        for (int i = 0; i < historyList.size(); ++i) {
            ...
            for (TopExp_Explorer exE(rec.occShape, TopAbs_EDGE); exE.More(); exE.Next()) {
                ...
                for (int s = 0; s <= 4; ++s) {
                    const double t = f + (l - f) * (static_cast<double>(s) / 4.0);
                    consider(curve->Value(t), i);
                }
            }
        }
        if (bestIdx >= 0) return bestIdx;
