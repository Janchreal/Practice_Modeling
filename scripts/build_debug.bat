...
%QT_DIR%\bin\qmake.exe %SRC_DIR%\Modeling.pro -spec win32-msvc  "CONFIG+=debug" "CONFIG+=console"
if exist %BUILD_DIR%\debug\Modeling.exe del %BUILD_DIR%\debug\Modeling.exe
nmake Debug
if not exist %BUILD_DIR%\debug\Qt5Cored.dll (
  %QT_DIR%\bin\windeployqt.exe %BUILD_DIR%\debug\Modeling.exe
)