...
#include <vtkRenderWindowInteractor.h>
#include <vtkInteractorStyleTrackballCamera.h>
#include <BRepAlgoAPI_Fuse.hxx>    // 布尔并集
#include <BRepAlgoAPI_Common.hxx>  // 布尔交集
#include <BRepAlgoAPI_Cut.hxx>     // 布尔差集
...
// 布尔 OCC 运算（widget_boolean.cpp 实现；模型再生等共用）
...
// MouseInteractorStyle 与 widget.cpp 之间共享：模态对话框等打开 VTK 交互时重置点击/拖拽判定
...
public:
    ...
    /** 实体模型材质：配合场景光照的 Phong 参数 */
    static void applySolidActorMaterial(class vtkProperty* prop);
    static vtkSmartPointer<IVtkTools_DisplayModeFilter>
    configureSolidShapePipeline(IVtkTools_ShapeDataSource* source,
                                class vtkDataSetMapper* mapper,
                                class vtkActor* actor);
    ...
    // 添加这些公共函数供表达式对话框访问
    const QList<ModelingHistory>& getHistoryList() const { return historyList; }
...
private slots:
    ...
    void performFillet(double radius);
...
private:
    friend class SketchEditCommand;
    Ui::Widget *ui;
    // 文件（文档）状
     ...
     *  L1 featureSelectionOverlayRenderer_ — 覆盖外观：高亮/选中/预览
     *  L2 referenceOverlayRenderer_        — 参考：矢量/点操作柄、坐标系轴
     *  L3 centerAxesRenderer               — 视图三重轴
     */
    vtkSmartPointer<vtkRenderer> centerAxesRenderer;
    vtkSmartPointer<vtkRenderer> referenceOverlayRenderer_;
    vtkSmartPointer<vtkCallbackCommand> refOverlayAlwaysOnTopObs_;
    vtkSmartPointer<vtkCallbackCommand> featureOverlayAlwaysOnTopObs_;
    void configureOverlayLayerAlwaysOnTop(vtkRenderer* overlay,
    ...
    /** 按当前相机视线剔除双背面棱，并刷新已有轮廓几何 */
    void updateModelBoundaryOutlinesForView();
    bool currentCameraViewDir(gp_Dir& outDir) const;
    void applyOutlinePolyData(int index, vtkSmartPointer<vtkPolyData> edges);
    /** 实体面略后推，配合轮廓线深度测试，避免遮挡边“穿透”显示 */
    static void configureSolidMapperForBoundaryOutline(class vtkMapper* mapper);
    /** 轮廓线开启深度检测、不做朝向相机的强偏移 */
    static void configureOutlineMapperHiddenLine(class vtkMapper* mapper);
    /** 旋转轴矢量箭头起点：旋转中心球心（剖面中心在轴上的投影） */
 ...
 private:
    ...
    Standard_Real vectorDialogCurveFirst_ = 0.0;
    Standard_Real vectorDialogCurveLast_ = 0.0;
    ...
    void applyOriginFromSnap(const gp_Pnt& p, const QString& chosenLabel);
     ...
     // OpenCASCADE 几何体创建函数
    TopoDS_Shape createCuboidShape(double length, double width, double height,
     ...
     TopoDS_Shape createCylinderShape(double radius, double height,
                                      bool hasOrigin = false, double originX = 0.0, double originY = 0.0, double originZ = 0.0,
                                      AxisDirection axisDirection = AxisDirection::Z,
                                      bool axisReversed = false,
                                      bool hasCustomVectorDir = false,
                                      const gp_Dir& customVectorDir = gp_Dir(0, 0, 1));
     TopoDS_Shape createConeShape(double radius1, double radius2, double height,
                                  bool hasOrigin = false, double originX = 0.0, double originY = 0.0, double originZ = 0.0,
                                  AxisDirection axisDirection = AxisDirection::Z,
                                  bool axisReversed = false,
                                  bool hasCustomVectorDir = false,
                                  const gp_Dir& customVectorDir = gp_Dir(0, 0, 1));
     TopoDS_Shape createSphereShape(double radius,
                                    bool hasOrigin = false, double originX = 0.0, double originY = 0.0, double originZ = 0.0);
     // 新增：获取选中形状的函数
    ...
    void updateModelName(int index);
     ...
     const int MAX_UNDO_STACK_SIZE = 50; // 最大撤销步数
 ...
 public:
    ...
    void createCylinderWithParams(double radius, double height,
     ...
     TopoDS_Shape buildLinearPatternShape(const QList<int>& sourceIndices,
                                          ...
                                          double pitch2,
                                          int count2) const;
     TopoDS_Shape buildCircularPatternShape(const QList<int>& sourceIndices,
                                            const gp_Pnt& origin,
                                            ...
                                            double radialPitch,
                                            int count2) const;
     TopoDS_Shape buildPolygonalPatternShape(const QList<int>& sourceIndices,
                                             const gp_Pnt& origin,
                                             const gp_Dir& axisDir,
                                             double spanDegrees,
                                             int sides,
                                             ...
                                             double radialPitch,
                                             int count2) const;
     TopoDS_Shape buildPatternShape(PatternFeatureDialog* dialog,
     ...
     void performBooleanOperation(int targetIndex,
                                  ...
                                  bool keepTool);
     // 模型管理方法
     int getHistorySize() const { return historyList.size(); }
     void removeModel(int index);
     void showModel(int index);
     // 命令系统专用方法（供命令类调用）
     int createGeometryDirectly(ModelType type, const QString& name, const QColor& color,
 ...
 private:
    ...
    static bool buildRevolutionCompound(const QList<ExtrusionFaceSelection>& selections,
                                         const gp_Ax1& axis,
                                         double angleRad,
    ...
    QList<vtkSmartPointer<vtkActor>> extrusionFaceHighlightActors; // 选中面高亮actors列表
     ...
     /** 拉伸/旋转选择高亮专用叠加层（L1 覆盖外观），避免被幽灵半透明体遮挡 */
     vtkSmartPointer<vtkRenderer> featureSelectionOverlayRenderer_;
