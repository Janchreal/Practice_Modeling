// 文档：新建/打开/保存、最近文件、自动恢复（从 widget.cpp 拆出）
#include "widget.h"
#include "ui_widget.h"
...
static QJsonArray historyListToJson(const QList<ModelingHistory>& historyList)
{
    QJsonArray models;
    for (const ModelingHistory& rec : historyList) {
...
bool Widget::saveDocumentToPath(const QString& filePath)
{
    ...
    root["models"] = historyListToJson(historyList);
...
bool Widget::loadDocumentFromPath(const QString& filePath)
{
    ...
    for (const QJsonValue& v : models) {
        ...
        const bool hasOrigin = m.value("hasOrigin").toBool(false);
        const double ox = m.value("originX").toDouble(0.0);
        const double oy = m.value("originY").toDouble(0.0);
        const double oz = m.value("originZ").toDouble(0.0);
        const AxisDirection axis = static_cast<AxisDirection>(m.value("axisDirection").toInt(2));
        const bool axisReversed = m.value("axisReversed").toBool(false);
        const bool hasCustomDir = m.value("hasCustomVectorDir").toBool(false);
        const double dx = m.value("dirX").toDouble(0.0);
        const double dy = m.value("dirY").toDouble(0.0);
        const double dz = m.value("dirZ").toDouble(1.0);
        const double dLen2 = dx * dx + dy * dy + dz * dz;
        const gp_Dir dir = (dLen2 > 1e-12) ? gp_Dir(dx, dy, dz) : gp_Dir(0, 0, 1);
        ...
        if (!brepText.isEmpty()) {
            ...
            if (!restored.IsNull()) {
                ...
                restoreModel(insertAt, name, type, color, p1, p2, p3, restored,
                             hasOrigin, ox, oy, oz, axis, axisReversed,
                             hasCustomDir, dir);
                if (insertAt >= 0 && insertAt < historyList.size() && historyList[insertAt].actor) {
                    historyList[insertAt].actor->SetVisibility(visible ? 1 : 0);
                }
        ...
        if (type == CUBOID || type == CYLINDER || type == CONE || type == SPHERE) {
            int idx = createGeometryDirectly(type, name, color, p1, p2, p3,
                                             hasOrigin, ox, oy, oz, axis, axisReversed,
                                             hasCustomDir, dir);
            if (idx >= 0 && idx < historyList.size() && historyList[idx].actor) {
                historyList[idx].actor->SetVisibility(visible ? 1 : 0);
            }
            ...
            restoreModel(insertAt, name, type, color, p1, p2, p3, TopoDS_Shape(),
                         false, 0.0, 0.0, 0.0, AxisDirection::Z, false, false, gp_Dir(0, 0, 1));
            applyHistoryMetadataFromJson(insertAt, m);
