// 矢量对话框 / 方向拾取 + 捕捉点 UI 与拾取（从 widget.cpp 拆出）
#include "widget.h"
#include "ui_widget.h"
                                            ...
                                            IVtk_IdType* outHoverSubShapeId)
{
    ...
    auto computeEdgeTangent = [&]() -> bool {
        ...
        for (IVtk_ShapeIdList::Iterator sIt(subShapeIds); sIt.More(); sIt.Next()) {
            ...
            Standard_Real evalParam = (firstParam + lastParam) / 2.0;
            if (modeIndex == 4) {
                try {
                    GeomAdaptor_Curve adaptor(curve, firstParam, lastParam);
                    const Standard_Real totalLen = GCPnts_AbscissaPoint::Length(adaptor, firstParam, lastParam);
                    vectorDialogCurveEdge_ = edge;
                    hasVectorDialogCurveEdge_ = true;
                    vectorDialogCurveFirst_ = firstParam;
                    vectorDialogCurveLast_ = lastParam;
                    vectorDialogCurveTotalLen_ = (totalLen > 0.0 ? static_cast<double>(totalLen) : 0.0);

                    Standard_Real s = 0.0;
                    if (vectorDialogCurvePosMode_ == 0) { // %
                        const Standard_Real pct = std::max<Standard_Real>(0.0, std::min<Standard_Real>(100.0, vectorDialogCurvePosValue_));
                        s = totalLen * (pct / 100.0);
                    } else { // mm
                        s = std::max<Standard_Real>(0.0, std::min<Standard_Real>(totalLen, vectorDialogCurvePosValue_));
                    }
...
void Widget::openVectorDialog(int desiredModeIndex)
{
    ...
    auto recomputeCurveVectorIfPossible = [this]() {
        ...
        Standard_Real firstParam = vectorDialogCurveFirst_;
        Standard_Real lastParam = vectorDialogCurveLast_;
        Handle(Geom_Curve) curve = BRep_Tool::Curve(vectorDialogCurveEdge_, firstParam, lastParam);
        ...
        try {
            ...
            GCPnts_AbscissaPoint ab(adaptor, s, firstParam);
            if (ab.IsDone()) evalParam = ab.Parameter();
        } catch (...) {
        }
        ...
        gp_Vec d1;
        curve->D1(evalParam, p, d1);
        if (d1.Magnitude() <= Precision::Confusion()) return;
...
{
    // UI 上的捕捉点按钮来自 widget.ui（点选择过滤器区域）
    // 将它们设置为可切换，并绑定到内部状态。
    if (!ui) return;
...
static bool snapProjectRayOntoEdge(const TopoDS_Edge& edge, const gp_Lin& ray, gp_Pnt& outPoint)
{
    ...
    try {
        ...
        GeomAPI_ProjectPointOnCurve proj(onCurve, curve, f, l);
        if (proj.NbPoints() >= 1) {
            outPoint = proj.NearestPoint();
        } else {
            outPoint = onCurve;
        }
        return true;
                                                    ...
                                                    QList<SketchSnapScreenCandidate>& out) const
{
    ...
    for (const TopoDS_Edge& edge : edges) {
        ...
        if (snap_.arcMidpoint) {
            Handle(Geom_Curve) basisAm = curve;
            if (basisAm->IsKind(STANDARD_TYPE(Geom_TrimmedCurve))) {
                basisAm = Handle(Geom_TrimmedCurve)::DownCast(basisAm)->BasisCurve();
            }
            if (!Handle(Geom_Circle)::DownCast(basisAm).IsNull()) {
...
void Widget::updateSnapHover(int x, int y)
{
    ...
    auto collectFromPickedEdges = [&](const IVtk_ShapeIdList& subShapeIds, const Handle(IVtkOCC_Shape)& shapeWrapper) {
        ...
        for (const TopoDS_Edge& edge : edges) {
            ...
            if (snap_.arcMidpoint) {
                // arc midpoint 语义：只在曲线本质为圆时，返回“参数中点”
                Handle(Geom_Curve) basis = curve;
                if (basis->IsKind(STANDARD_TYPE(Geom_TrimmedCurve))) {
                    basis = Handle(Geom_TrimmedCurve)::DownCast(basis)->BasisCurve();
                }
        ...
        if (snap_.intersection && edges.size() >= 2) {
            ...
            for (int i = 0; i < edges.size(); ++i) {
                for (int j = i + 1; j < edges.size(); ++j) {
                    ...
                    GeomAPI_ExtremaCurveCurve extrema(c1, c2);
                    // OCCT 7.7：GeomAPI_ExtremaCurveCurve 没有 IsDone()，失败时 NbExtrema() 返回 0，
                    // 且 NearestPoints()/LowerDistance() 可能抛 StdFail_NotDone。
                    if (extrema.NbExtrema() < 1) continue;
                    gp_Pnt p1, p2;
                    try {
                        extrema.NearestPoints(p1, p2);
                    ...
                    const Standard_Real d = p1.Distance(p2);
                    if (d <= tol) {
                        gp_Pnt pi((p1.X() + p2.X()) * 0.5, (p1.Y() + p2.Y()) * 0.5, (p1.Z() + p2.Z()) * 0.5);
        ...
        for (const auto& rec : historyList) {
            ...
            if (snap_.midpoint) {
                for (TopExp_Explorer exE(rec.occShape, TopAbs_EDGE); exE.More(); exE.Next()) {
                    ...
                    const gp_Pnt mid = curve->Value((f + l) * 0.5);
                    candidates.append({tr("中点"), mid, snapScreenDist2(renderer, mid, x, y), e, true});

                    // 圆心/象限点（仅基于有限边扫描，避免复杂交点计算）
                    if (snap_.center || snap_.quadrant) {
                        Handle(Geom_Curve) basis = curve;
                        if (basis->IsKind(STANDARD_TYPE(Geom_TrimmedCurve))) {
                            basis = Handle(Geom_TrimmedCurve)::DownCast(basis)->BasisCurve();
                        }
                        ...
                        if (!circ.IsNull()) {
                            const gp_Pnt c = circ->Location();
                            if (snap_.center) {
                                candidates.append({tr("圆心"), c, snapScreenDist2(renderer, c, x, y), e, true});
                            }
                            if (snap_.quadrant) {
                                const gp_Ax2 ax = circ->Position();
                                const gp_Dir xd = ax.XDirection();
                                const gp_Dir yd = ax.YDirection();
                                const double r = circ->Radius();
                                candidates.append({tr("象限点"), gp_Pnt(c.X() + xd.X() * r, c.Y() + xd.Y() * r, c.Z() + xd.Z() * r), snapScreenDist2(renderer, gp_Pnt(c.X() + xd.X() * r, c.Y() + xd.Y() * r, c.Z() + xd.Z() * r), x, y), e, true});
                                candidates.append({tr("象限点"), gp_Pnt(c.X() - xd.X() * r, c.Y() - xd.Y() * r, c.Z() - xd.Z() * r), snapScreenDist2(renderer, gp_Pnt(c.X() - xd.X() * r, c.Y() - xd.Y() * r, c.Z() - xd.Z() * r), x, y), e, true});
                                candidates.append({tr("象限点"), gp_Pnt(c.X() + yd.X() * r, c.Y() + yd.Y() * r, c.Z() + yd.Z() * r), snapScreenDist2(renderer, gp_Pnt(c.X() + yd.X() * r, c.Y() + yd.Y() * r, c.Z() + yd.Z() * r), x, y), e, true});
                                candidates.append({tr("象限点"), gp_Pnt(c.X() - yd.X() * r, c.Y() - yd.Y() * r, c.Z() - yd.Z() * r), snapScreenDist2(renderer, gp_Pnt(c.X() - yd.X() * r, c.Y() - yd.Y() * r, c.Z() - yd.Z() * r), x, y), e, true});
                            }
                        }
                    }
                        ...
                        if (snap_.intersection && edges.size() >= 2) {
                            ...
                            for (int i = 0; i < edges.size(); ++i) {
                                for (int j = i + 1; j < edges.size(); ++j) {
                                    ...
                                    if (p1.Distance(p2) <= tol) {
                                        addCandidate(tr("交点"), gp_Pnt((p1.X() + p2.X()) * 0.5, (p1.Y() + p2.Y()) * 0.5, (p1.Z() + p2.Z()) * 0.5));
                                    }
                                }
                            }
                        }
